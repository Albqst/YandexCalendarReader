...
public class ReadYandex
{
    ...
    try
    {
        ...
        var client = new WebDavClient(new WebDavClientParams
        {
            ...
            Credentials = new NetworkCredential("iroromani@yandex.ru", settings.PasswordAlbert)
        });

        var calendarsPath = "/calendars/iroromani@yandex.ru/";
        var result = await client.Propfind(calendarsPath);

        settings.CalendarUri =
            Loader.GetTargetCalendarUri(
                result,
                settings.CalendarUri,
                settings.NameOfCalendar);

        // 3. Получаем события
        var responseXmlList = await Loader.GetCalendarEvents(
            settings.CalendarUri,
            "iroromani@yandex.ru",
            settings.PasswordAlbert);
    ...
    // public async Task<bool> ReadAsync(AppSettings settings)
    // {
    //     try
    //     {
    //         Console.WriteLine("===== Яндекс Календарь Reader =====\n");
