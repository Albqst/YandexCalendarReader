...
class Program
{
    static async Task Main()
    {
        ...
        try
        {
            ...
            var loader = new Load();
            calendarUri = loader.GetTargetCalendarUri(result, calendarUri); // Получили ссылку на целевой календарь с событиями
            var responseXmlString = await loader.GetCalendarEvents(calendarUri, "iroromani@yandex.ru", PasswordAlbert);
            
            // string calendarData2 = "...твой текст между BEGIN:VCALENDAR ... END:VCALENDAR...";
            var parser = new CalendarEvent();
            var evt = parser.ParseVEvent(responseXmlString);
...
public class CalendarEvent
{
    ...
    private DateTime ParseDateTime(string line)
    {
        ...
        if (match.Success && DateTime.TryParseExact(match.Groups[1].Value, "yyyyMMdd'T'HHmmss", null, System.Globalization.DateTimeStyles.AssumeLocal, out var dt))
        {
            return dt;
        }
...
public class Load
{
    ...
    public async Task<string> GetCalendarEvents(string calendarUri, string login, string appPassword)
    {
  ...
  <C:filter>
    <C:comp-filter name=""VCALENDAR"">
      <C:comp-filter name=""VEVENT"">
        <C:time-range start=""20250514T000000Z"" end=""20250515T000000Z""/>
      </C:comp-filter>
    </C:comp-filter>
  </C:filter>
</C:calendar-query>";
